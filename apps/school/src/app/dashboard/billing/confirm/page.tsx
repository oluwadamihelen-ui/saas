...
/// (Winfield subscription) invoice — mirrors /pay/[token]/confirm's real-
    ...
    <div className="max-w-lg">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isRealGateway ? `Paid via ${invoice.provider}` : "This is a simulated checkout — Winfield hasn't connected a live payment gateway yet."}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-2xl font-semibold text-foreground">{formatMoney(invoice.amountMinor, invoice.currency)}</p>
          <p className="text-sm text-muted">
            For your {invoice.billingInterval === "YEARLY" ? "annual" : "monthly"} Winfield subscription
          </p>
