...
import { startCharacterGeneration } from "@cinerra/domain";
...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { generationJobId } = await startCharacterGeneration({
      userId,
      projectId: params.id,
      enqueue: (jobId) => enqueueGenerationJob(QUEUE_NAMES.characterGeneration, jobId, bullPriorityFor(policy.queuePriority)),
    });
