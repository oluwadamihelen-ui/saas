...
import { startPropGeneration } from "@cinerra/domain";
...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { generationJobId } = await startPropGeneration({
      userId,
      projectId: params.id,
      enqueue: (jobId) => enqueueGenerationJob(QUEUE_NAMES.propGeneration, jobId, bullPriorityFor(policy.queuePriority)),
    });
