...
import { startSoundEffectGeneration } from "@cinerra/domain";
...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { generationJobId } = await startSoundEffectGeneration({
      userId,
      shotId: params.id,
      enqueue: (jobId) => enqueueGenerationJob(QUEUE_NAMES.audioGeneration, jobId, bullPriorityFor(policy.queuePriority)),
    });
