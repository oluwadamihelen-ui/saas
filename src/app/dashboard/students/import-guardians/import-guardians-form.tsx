...
import {
  previewGuardianImportAction,
  confirmGuardianImportAction,
  type GuardianImportPreviewState,
  type GuardianImportConfirmState,
} from "./actions";

export function GuardianImportForm() {
  const router = useRouter();
  const [previewState, previewAction, isPreviewing] = useActionState(
    previewGuardianImportAction,
    { status: "idle" } as GuardianImportPreviewState
  );
  useActionToast(previewState);
  const [confirmState, confirmAction, isConfirming] = useActionState(
    confirmGuardianImportAction,
    { status: "idle" } as GuardianImportConfirmState
  );
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-foreground">
          Added <strong>{confirmState.created}</strong> guardian{confirmState.created === 1 ? "" : "s"}.
        </p>
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => router.refresh()}>Add more guardians</Button>
          <Button variant="secondary" onClick={() => router.push("/dashboard/students")}>Back to students</Button>
        </div>
      </div>
    );
  }
      ...
      {previewState.status === "previewed" && previewState.rows && (
        <div className="space-y-4">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Admission No. — Guardian</TableHead>
          ...
          <form action={confirmAction} className="flex items-center gap-3">
            ...
            <input type="hidden" name="fileName" value={previewState.fileName ?? "guardians.csv"} />
            <Button type="submit" disabled={isConfirming || validCount === 0}>
              {isConfirming ? "Importing..." : `Add ${validCount} guardian${validCount === 1 ? "" : "s"}`}
            </Button>
