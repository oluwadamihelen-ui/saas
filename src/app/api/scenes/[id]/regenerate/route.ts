...
import { getSceneImageQueue } from "@/server/jobs/queue";
...
export async function POST(req: Request, { params }: RouteParams) {
  try {
    ...
    if (!rateLimit(requestKey(req, `regenerate:${userId}`), 20, 60_000)) {
      return NextResponse.json({ error: "Too many requests. Please try again shortly." }, { status: 429 });
    }

    const scene = await getSceneForUser(userId, id);

    await prisma.scene.update({ where: { id }, data: { imageStatus: "QUEUED" } });
    await getSceneImageQueue().add(
