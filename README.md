...
Terms of Service and Privacy Policy pages now exist at `/terms` and `/privacy` (linked from a new site footer), and signup requires accepting both — recorded as `User.termsAcceptedAt`, never backfilled or assumed. Both are explicitly marked as a template draft for a lawyer to review, not legal advice. Google OAuth sign-in (when configured) creates a user via NextAuth's own adapter, bypassing this checkbox — closed by gating every authenticated page behind `requireAcceptedTerms()` (`apps/web/src/lib/authGuards.ts`), which sends anyone with no recorded acceptance to `/accept-terms` before they can use anything else. Deliberately page-level, not global middleware: Next.js middleware defaults to the Edge runtime, and this project's Prisma client is the standard Node engine, not an edge-compatible driver adapter — a Prisma-querying middleware.ts would risk breaking at deploy time in a way that's easy to miss locally. Page-level checks match how every other auth/role gate in this app already works (see e.g. `/admin`'s own `role !== "ADMIN"` check) and run in the Node runtime where Prisma is guaranteed to work.
...
- ~~**Fraud/risk controls**~~ — built the two pieces that were both realistic to compute from data this app actually has and safe to ship without a live fraud-ops team: risk *signals*, and a *suspension* that actually works. `apps/web/src/lib/trustSafety.ts`'s `getFraudSignals()` computes two real heuristics from `CoinPurchase` history — a high refund rate (3+ purchases, 50%+ refunded) and unusually fast repeat purchasing (5+ in 24h) — surfaced on `/admin` under "Risk & Trust" for a human to review, not to act on automatically; self-purchase (a creator watching their own paid content) is still deliberately not flagged, since no money moves. The bigger find here wasn't the signals, though — it was that `AccountStatus.SUSPENDED` had existed in the schema since the very first phase and was checked at login, but nothing had ever set it, and even if something had, an already-issued NextAuth JWT would have kept working regardless: `session()` in `auth.ts` only populated `role` per request, never re-checked `status`. Both are fixed: `suspendUser`/`unsuspendUser` (admin-only, audit-logged via the existing `AuditLog` model) actually set the status now, and `session()` re-fetches `status` on every request so a suspension cuts off an already-logged-in session on its very next API call — not just at its next login. `requireUserId`/`requireAdmin`, the single choke point all 55 authenticated API routes already go through, now reject a non-`ACTIVE` account with a distinct "account suspended" response rather than a generic sign-in prompt. Verified against real Postgres: a 4-purchase/50%-refunded account is flagged and a 3-purchase/33%-refunded one isn't; a 6-purchases-in-an-hour account is flagged and one spread over three days isn't; old purchases outside the 24h window don't trigger the velocity signal; suspending sets the status and writes the audit row; unsuspending restores it. Explicitly still out of scope: device/IP collusion signals (no such data is collected anywhere in this app), and page-level redirects — a suspended user's server-rendered pages (e.g. `/wallet`) still render if requested directly, since the enforcement point is the API routes that actually move money or mutate state, not page loads.
...
1. Set `PlatformSettings.payoutCoinValueCents` and `payoutCurrency` — the coin-to-fiat conversion rate paid out to creators — from admin → Doe Economy tab, alongside the platform's other revenue-share and price-range settings.
...
**Generation metering (Doe cost recovery)** — every AI provider call that costs real money charges the creator Doe, not just viewers unlocking content:
1. `PlatformSettings.doeCostPerReferenceImage` / `doeCostPerVideoSecond` (admin → Doe Economy tab) set what image/video generation costs in Doe; defaults assume ~$0.06/image and ~$0.05/video-second (OpenAI image gen, Runway Gen-4 Turbo) at the platform's own $0.01/Doe rate, with margin built in.
2. `PlatformSettings.doeCostPerTextGeneration` charges a flat rate for each story/screenplay/character/location/prop bible generation call (one LLM call each — sized off Claude Sonnet's ~$3/$15 per-M-token in/out pricing at a typical few-thousand-token call, ~$0.05, with margin). `doeCostPerVoice100Chars` charges dialogue voice synthesis per 100 characters of spoken text (ElevenLabs-style ~$0.05–0.10/1k chars). `doeCostPerAudioSecond` charges sound-effect and music generation per second of generated audio (both land in the same rough $0.002–0.0025/sec range, so they share one rate rather than two near-identical ones). All three are editable from the same admin Doe Economy tab as the image/video rates.
3. `Plan.includedGenerationDoe` is each plan's monthly allowance, editable from admin → Doe Economy tab → "Plan Doe allowances," granted as an expiring PromotionalGrant — paid subscribers get it on the Paystack webhook when a new billing period starts; a user with no active subscription gets the Free plan's allowance lazily, once per rolling 30-day window, on their first generation attempt.
4. Every metered call site charges *before* the paid provider call and refunds on failure (`chargeForGeneration`/`refundGenerationCharge` in `packages/database/src/generationBilling.ts`) — reference images, shot video, story/script/character/location/prop text, dialogue voice, sound effects, and music are all covered.
5. Deliberately NOT metered, and not a gap: the automated QC vision pass that runs after shot generation, and episode/trailer/social-clip export. QC's few IMAGE_ANALYSIS calls per shot are cheap enough to fold into the video generation cost above rather than add a fourth line item; export/clip generation runs pure FFmpeg over shots that were already charged when *they* were generated — it makes no further paid provider call, so there is nothing to recover a cost for.
6. Re-check every one of these defaults against your actual provider pricing before going live — they move, and the numbers above are this project's best real-cost research, not a guarantee.
