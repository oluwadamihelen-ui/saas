...
**Remaining before launch** (not part of the original phased roadmap, but needed for a real public launch): self-serve account deletion, closing the Google-OAuth terms-acceptance gap noted above, real ESLint setup (`next lint` has never been configured, per the CI note above), and client-side error capture (noted above).
