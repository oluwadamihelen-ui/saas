...
**Remaining before launch** (not part of the original phased roadmap, but needed for a real public launch): transactional email (today `Notification` rows are in-app only), self-serve account deletion, closing the Google-OAuth terms-acceptance gap noted above, real ESLint setup (`next lint` has never been configured, per the CI note above), and client-side error capture (noted above).
