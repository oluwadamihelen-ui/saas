...
**Two real bugs were found and fixed by that testing, not by inspection** — worth calling out because both are exactly the kind of subtle correctness issue a financial system can't ship with:
...
**Known, unresolved gap** (flagged rather than silently shipped): `deleteUserAccount` (from the earlier account-deletion phase) deletes a publisher's `Project` rows, which cascades away `ContentEntitlement`/`RevenueTransaction`/`CreatorEarning` for *every viewer* who ever bought that publisher's content — destroying other people's purchase history and the platform's own revenue records, which the coin economy's design otherwise treats as permanent. This is a real interaction between two features built in different phases that isn't resolved yet; a publisher with monetized content and real viewer revenue history probably shouldn't be able to self-delete without a fix here first (documented in code at both `deleteUserAccount` and the `RevenueTransaction` model).
