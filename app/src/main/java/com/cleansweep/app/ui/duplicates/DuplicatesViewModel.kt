...
class DuplicatesViewModel(
    ...
    private val _scanState = MutableStateFlow<ScanState<List<DuplicateGroup>>>(ScanState.Idle)
    val scanState: StateFlow<ScanState<List<DuplicateGroup>>> = _scanState

    /** File ids the user has selected for deletion; pre-seeded with each group's suggestion. */
    ...
    fun scan() {
        viewModelScope.launch {
            ...
            runCatching { scanner.findExactDuplicates() }
                .onSuccess { groups ->
                    _scanState.value = ScanState.Success(groups)
                    _selectedIds.value = groups.flatMap { it.suggestedDelete }.map { it.id }.toSet()
                }
    ...
    fun deleteSelected() {
        val groups = (_scanState.value as? ScanState.Success)?.result ?: return
        val files = groups.flatMap { it.files }.filter { it.id in _selectedIds.value }
    ...
    /** Call after the consent Activity result comes back with RESULT_OK. */
